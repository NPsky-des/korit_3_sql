...
2. js_sql_study에서 새 폴더 만들기 > js_study 생성
3. ch01 폴더 생성
4. 새 파일 > index.html 생성
5. live server 실행
6. ctrl + shift + i > 개발자도구
...
```
콘솔 기능의 장점은 괄호 내에 쉼표(,)를 이용하여 여러 데이터를 한 번에 출력이 가능하다는 점.
...
* 예전에는 변수를 선언할 때 var를 썼습니다.
  - ES6 이전에는 JS에서 변수를 선언할 때 var를 사용. 근데 ES6버전 이후에는 let과 const가 var를 대체했습니다.

```js
// 여기가 코드의 처음이라고 가정합니다.
console.log(b);   //원래라면 오류 발생해야 함. 변수 b가 없기 때문// undefined
var b = 3;        // 근데 var로 선언됐기 때문에 오류가 발생하지 않고 ↑
                  // 근데 56번 라인에 3이 아니라 undefined인 이유는
                  // b에 3이라는 데이터가 저장된 시점이 57번 라인이기 때문
                  // 근데 이상한 점은 변수 선언도 57번이라는 점.
                  // 그게 JS가 일관성 없는 프로그래밍 언어인 이유였습니다.
                  // 이상을 해결하기 위해 let / const가 등장했습니다.
console.log(a);   // 얘도 56번 라인과 차이가 없어보이는데,
...
```js
let studentName = '이정화';   // 변수 선언 및 초기화
console.log(studentName);    //결과값 : 이정화
studentName = '김일';         // 선언된 변수에 데이터 재대입 -> let 명시X
console.log(studentName);    // 결과값: 김일
```
...
- 이상의 경우가 개발자 입장에서 변수 개수를 줄이면서 데이터를 바꿀 수 있으니 좋다고 생각하실 수도 있는데 예를 들어 input 태그에 type을 number로 잡아놓고 수학 연산을 하려 했는데 거기에 String 자료형이 들어오게 되면 페이지에 오류가 발생할 수밖에 없습니다. 그런데 대입 자체까지는 오류가 발생하지 않기 때문에 개발자들이 어느 부분에서 오류가 발생했는지 정확하게 파악하기 힘들다는 점이 문제였습니다.
...
console.log(x, typeof x);   // 결과값: undefiend 'undefiend'
...
```
특정 데이터의 자료형이 null인지는 확인이 불가능하고 x === null; 같이 추가적으로 코드를 작성해야 한다는 문제점이 있습니다.

왜 object로 뜨는가?
- JS 자체가 땜질해가면서 만든 언어다보니까 초기 설계가 부실해서 최상위 계층인 object로 뜨는 겁니다.

그렇다면 추후 사후 보강하면 되지 않는가?
- JS가 원래는 웹사이트에서 사용하는 언어이다보니 근본을 전부 바꾸어버리면 JS 기반으로 만든 웹사이트들이 전부 문제가 생깁니다. 그래서 기존의 문법을 바꾸는 대신 더 좋은 기능을 추가해서 앞으로 개발할 때는 새로운 것을 이용하도록 하는 형태로 장려하고 있습니다.
 - let / const가 생겼는데 여전히 var가 있는 이유
...
- 할당 산술 연산자
  ...
  - x /= y
  - x %= y
  - x ** y      - x 값을 y만큼 제곱한 값을 구함. (Java에선 못 본 예)
...
console.log(
  '당신의 BMI 지수는 ' + (weight/(height/100)*2) + '입니다.'
);
```
...
let height = 172.5;
let weight = 68.7;
let bmi;      // 아직 대입 안 시킬 거니까 선언만 했습니다.
let answerBmi;
//일단 height가 cm이기 때문에 m으로 바꿔줘야 함.
height /= 100;
bmi = weight / (height**2);
...
```
이상과 같은 방식 등으로 풀이가 가능합니다.(다양한 방법의 풀이가 있을 수 있는데 배운 것들을 최대한 사용하기 위해 저렇게 씀.)

여기서 중요한 점은 console.log(); 에 다양한 자료형의 데이터를 한꺼번에 작성하기 위해서는 '+', 혹은 ','가 연결되어 있어야 해서 불편합니다.
  -> Java를 배운 저희는 그냥 그러려니하고 작성했었지만 다른 Frontend 개발자들은 저 개념을 좀 싫어했습ㄴ다.
    그래서 다양한 자료형을 한 번에 넣더라도 +와 , 로 연결하지 않는 방법을 고안했는데, 그것이 백틱(`)을 이용한 방법입니다.

형식
```js
console.log(`어쩌고저쩌고 스트링데이터 : ${JS변수명}`);
```
으로 쓰게 될 경우 스트링 데이터와 JS 변수를 연결할 필요 없이 {} 내부 영역이 외부에 있는 변수를 참조한다는 것을 명시해줌으로써 가독성을 향상시켰습니다.
비슷한 경우가 python 등에서도 나타나기 때문에 알아두시는 게 향후 정신 건강에 좋습니다.


## 객체와 배열

### 객체(Object)
- 이상에서 배운 number, string, boolean 등의 자료형을 기본 자료형(Privitive types)이라고 했습니다. 즉, 각 자료형이 단 하나의 데이터만 가지고 있는 형태에 해당함.
...
public class PersonMain {
  public static main(args[]) {
    ...
    String key1 = scanner.nextLine();   //그리고 실행 시에 여러분이 married라고 쳤다고 가정
    System.out.println("김철수의 결혼 여부는 " + person1.key1 + "입니다.");
  }
}
```

java적으로 풀기는 했지만 이상의 코드는 오류가 발생합니다.
...
const person1Age = person1.age;   // 객체 property의 value 값을 하나의 변수에 대입하는 것이 가능
...
```js
const person1 = {
  name: '김일',               // 그 css할 때는 ; 썼는데 얘는 나열할 때 , 씁니다.
  age: 20,
  married: false,
};

person1.job = '프로그래머';     // #1방법
person1['bloodtype'] = 'B';   // #2방법
...
- const로 person1을 지정했는데, 왜 수정이 될까요?
  ...
  - 이걸 굳이 비유를 하자면
    - person1은 객체이면서, 하나의 집으로 규정한 것입니다. 따라서 집 자체를 바꾸는 것은 불가능하지만, 집의 내부에 있는 가구(값)들은 집이 바뀌지 않는 한 수정이 가능합니다.
...
const person1 = {
  name: '김일',
  age: 20,
  married: false,  // 이까지 썼을 때 맨 뒤에 ',' 찍어두는 게 개발자들 사이의 매너입니다.
};
...
// 요소 추가를 할 때는 Java와 다릅니다 => Java는 배열 크기가 고정돼있어서 추가가 불가능했죠.
...
4. 기술블로그에 마지막 종강일 전까지 몇 개 정도 올릴지.
